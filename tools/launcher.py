...
class MapManager:
    ...
    # Patterns for the Stanley toggle (mixer + stanley_avoidance).
    STANLEY_KILL_PATTERNS = [
        "f1tenth_mixer/lib/f1tenth_mixer/mixer_node\\.py",
        "f1tenth_controller/lib/f1tenth_controller/stanley_avoidance\\.py",
        "ros2 run f1tenth_mixer mixer_node",
        "ros2 run f1tenth_controller stanley_avoidance",
    ]
    ...
    # raceline_generator.py). The Lattice / Stanley buttons look up
    ...
    def __init__(self, parent, app):
        ...
        self.stanley_proc: Optional[subprocess.Popen] = None
        # Paused state: process stays alive (path/markers still publish), but
        # a Bool on /controller_pause forces commanded speed to 0 inside the
        # controller. Toggling the button cycles run ↔ pause; switching to a
        # different controller fully kills the previous one.
        self.fgm_paused = False
        self.lattice_paused = False
        self.stanley_paused = False
        self.fgm_speed_range = (0.5, 0.7)
        self.lattice_speed_range = (0.0, 0.7)
        self.stanley_speed_range = (0.5, 0.7)
        self.FGM_NODE = "/gap_follow"
        self.LATTICE_NODE = "/lattice_planner"
        self.STANLEY_NODE = "/stanley_avoidance_node"
        ...
        # Lattice toggle — needs a raceline matching the selected map
        # (path_v_<basename>.yaml). Lattice with no obstacle in range
        # degenerates to bicycle-model pure-pursuit, so there's no separate
        # Pure Pursuit button.
        ...
        # Stanley toggle — same raceline convention as Lattice.
        self.stanley_btn = tk.Button(
            auto, text="Stanley", width=8,
            font=("TkDefaultFont", 10, "bold"),
            command=self._toggle_stanley)
        self.stanley_btn.pack(side="left", padx=2)
        self.stanley_sublabel_var = tk.StringVar(value="")
        self.stanley_sublabel = tk.Label(
            auto, textvariable=self.stanley_sublabel_var,
            fg="#a72b29", font=("TkDefaultFont", 8))
        self.stanley_sublabel.pack(side="left", padx=(6, 0))
        self._stanley_default_bg = self.stanley_btn.cget("bg")
        self._stanley_default_fg = self.stanley_btn.cget("fg")
    ...
    def _stop_localize(self):
        if not self.is_localizing():
            return
        # Stopping loc tears down SLAM + sensors + actuator. Any FGM / PP /
        # Lattice that was running on top of it must also be killed, or it
        # keeps publishing /drive into the void with stale state. Reset
        # their button visuals too so the GUI matches reality.
        if self.is_fgm_running():
            self._stop_fgm()
        if self.is_lattice_running():
            self._stop_lattice()
        if self.is_stanley_running():
            self._stop_stanley()
        # Belt-and-suspenders: sweep any controller orphans that survived
        # (process group quirks, pkill race conditions, etc.).
        self._sweep_orphan_controllers(reason="loc base stopped")
        try:
    ...
    # ----- Pause / resume helpers (single Bool publish to all controllers) -

    def _set_pause(self, paused: bool, tag: str):
        """Publish a one-shot Bool to /controller_pause. The controllers
        subscribe with TRANSIENT_LOCAL durability so the latest message is
        re-delivered to a controller that just started. Runs in a thread
        because `ros2 topic pub --once` still takes ~1.5 s to start up."""
        def _run():
            cmd = chain(
                ROS_SETUP, ROS2_WS,
                f"ros2 topic pub --once "
                f"--qos-durability transient_local "
                f"--qos-reliability reliable "
                f"/controller_pause std_msgs/msg/Bool "
                f"'data: {str(paused).lower()}'",
            )
            try:
                subprocess.run(
                    ["bash", "-c", cmd],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                    env=clean_env(), timeout=8.0,
                )
                self.app.log_queue.put(
                    ("__system__", f"[{tag}] /controller_pause ← {paused}"))
            except subprocess.TimeoutExpired:
                self.app.log_queue.put(
                    ("__system__", f"[{tag}] pause publish timed out"))
            except Exception as e:
                self.app.log_queue.put(
                    ("__system__", f"[{tag}] pause publish failed: {e!r}"))
        threading.Thread(target=_run, daemon=True).start()

    # ----- Brake helper -------------------------------------------------

    def _brake_motor(self, reason: str = ""):
        """Publish zero RPM directly to /vesc/cmd a handful of times so the
        car physically stops after a controller is toggled off. Bypasses
        the mixer (which we've just killed) and goes straight to the VESC
        driver. Runs in a background thread so the GUI stays responsive."""
        tag = f" ({reason})" if reason else ""

        def _run():
            cmd = chain(
                ROS_SETUP, ROS2_WS,
                # 5 zeros at 20 Hz ≈ 0.25 s of explicit brake, enough to
                # overwrite the last commanded RPM from the dying mixer.
                "ros2 topic pub --times 5 --rate 20 /vesc/cmd "
            ...
            try:
                subprocess.run(
                    ["bash", "-c", cmd],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                    env=clean_env(), timeout=3.0,
                )
                self.app.log_queue.put(
                    ("__system__", f"[brake] zero RPM sent to /vesc/cmd{tag}"))
            except subprocess.TimeoutExpired:
                self.app.log_queue.put(
                    ("__system__", f"[brake] publish timed out{tag}"))
            except Exception as e:
                self.app.log_queue.put(
                    ("__system__", f"[brake] failed{tag}: {e!r}"))
    ...
    def _refresh_fgm_button(self):
        if self.is_fgm_running() and not self.fgm_paused:
            self.fgm_btn.configure(bg="#d11a2a", fg="white",
                                    activebackground="#a51220",
                                    activeforeground="white")
            self.fgm_sublabel_var.set("active")
        elif self.is_fgm_running() and self.fgm_paused:
            self.fgm_btn.configure(bg="#a78a17", fg="white",
                                    activebackground="#806710",
                                    activeforeground="white")
            self.fgm_sublabel_var.set("paused")
        else:
    ...
    def _toggle_fgm(self):
        # 3-state cycle: stopped → running → paused → running → paused → …
        # "Paused" means the controller process is alive (path/markers still
        # publishing in RViz) but min_speed=max_speed=0 so the car holds 0.
        # Switching to a different controller fully kills FGM.
        if not self.is_fgm_running():
            self._start_fgm()
        elif self.fgm_paused:
            self._resume_fgm()
        else:
            self._pause_fgm()

    def _pause_fgm(self):
        self._set_pause(True, tag="FGM")
        self._brake_motor(reason="FGM pause")
        self.fgm_paused = True
        self.app.log_system("[FGM] paused (speed=0, path still publishing)")
        self._refresh_fgm_button()

    def _resume_fgm(self):
        self._set_pause(False, tag="FGM")
        self.fgm_paused = False
        self.app.log_system("[FGM] resumed")
        self._refresh_fgm_button()
    ...
    def _start_fgm(self):
        ...
        if self.is_lattice_running():
            ...
            return
        if self.is_stanley_running():
            messagebox.showwarning("Stanley running",
                                    "Stop Stanley before starting FGM.")
            return
        ...
        # Clear any stale latched pause=True from a prior session so the
        # new controller doesn't immediately freeze on first scan.
        self._set_pause(False, tag="FGM-start")
    ...
    def _stop_fgm(self):
        ...
        self._brake_motor(reason="FGM stop")
    ...
    # ----- Raceline lookup shared by Lattice / Stanley ------------------
    ...
    def _refresh_lattice_button(self):
        if self.is_lattice_running() and not self.lattice_paused:
            self.lattice_btn.configure(bg="#0a3069", fg="white",
                                        activebackground="#082352",
                                        activeforeground="white")
            self.lattice_sublabel_var.set("active")
        elif self.is_lattice_running() and self.lattice_paused:
            self.lattice_btn.configure(bg="#a78a17", fg="white",
                                        activebackground="#806710",
                                        activeforeground="white")
            self.lattice_sublabel_var.set("paused")
        else:
    ...
    def _toggle_lattice(self):
        # 3-state cycle: stopped → running → paused → running → … (see _toggle_fgm).
        if not self.is_lattice_running():
            self._start_lattice()
        elif self.lattice_paused:
            self._resume_lattice()
        else:
            self._pause_lattice()

    def _pause_lattice(self):
        self._set_pause(True, tag="Lattice")
        self._brake_motor(reason="Lattice pause")
        self.lattice_paused = True
        self.app.log_system("[Lattice] paused (speed=0, path still publishing)")
        self._refresh_lattice_button()

    def _resume_lattice(self):
        self._set_pause(False, tag="Lattice")
        self.lattice_paused = False
        self.app.log_system("[Lattice] resumed")
        self._refresh_lattice_button()
    ...
    def _start_lattice(self):
        ...
        if self.is_stanley_running():
            messagebox.showwarning("Stanley running",
                                    "Stop Stanley before starting Lattice.")
            return
        self._sweep_orphan_controllers(reason="pre-Lattice")
        # Clear any stale latched pause=True so the new controller starts
        # in the running state, not stuck at speed 0.
        self._set_pause(False, tag="Lattice-start")
    ...
    def _stop_lattice(self):
        ...
        self._brake_motor(reason="Lattice stop")
    ...
    # ----- Stanley toggle (mixer + stanley_avoidance) -------------------

    def is_stanley_running(self) -> bool:
        return self.stanley_proc is not None and self.stanley_proc.poll() is None

    def _refresh_stanley_button(self):
        if self.is_stanley_running() and not self.stanley_paused:
            self.stanley_btn.configure(bg="#6c3a8e", fg="white",
                                        activebackground="#54286e",
                                        activeforeground="white")
            self.stanley_sublabel_var.set("active")
        elif self.is_stanley_running() and self.stanley_paused:
            self.stanley_btn.configure(bg="#a78a17", fg="white",
                                        activebackground="#806710",
                                        activeforeground="white")
            self.stanley_sublabel_var.set("paused")
        else:
            self.stanley_btn.configure(bg=self._stanley_default_bg,
                                        fg=self._stanley_default_fg,
                                        activebackground=self._stanley_default_bg,
                                        activeforeground=self._stanley_default_fg)
            self.stanley_sublabel_var.set("")

    def _toggle_stanley(self):
        if not self.is_stanley_running():
            self._start_stanley()
        elif self.stanley_paused:
            self._resume_stanley()
        else:
            self._pause_stanley()

    def _pause_stanley(self):
        self._set_pause(True, tag="Stanley")
        self._brake_motor(reason="Stanley pause")
        self.stanley_paused = True
        self.app.log_system("[Stanley] paused (speed=0, path still publishing)")
        self._refresh_stanley_button()

    def _resume_stanley(self):
        self._set_pause(False, tag="Stanley")
        self.stanley_paused = False
        self.app.log_system("[Stanley] resumed")
        self._refresh_stanley_button()

    def _start_stanley(self):
        if self.is_fgm_running():
            messagebox.showwarning("FGM running",
                                    "Stop FGM before starting Stanley.")
            return
        if self.is_lattice_running():
            messagebox.showwarning("Lattice running",
                                    "Stop Lattice before starting Stanley.")
            return
        self._sweep_orphan_controllers(reason="pre-Stanley")
        self._set_pause(False, tag="Stanley-start")
        raceline = self._raceline_for_selected()
        if raceline is None:
            return
        cmd = chain(
        ...
        self.stanley_proc = subprocess.Popen(
            ["bash", "-c", cmd],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            bufsize=1, text=True,
        ...
        self.app.log_system(
            f"[Stanley] started (pid {self.stanley_proc.pid}) — "
            f"mixer_node + stanley_avoidance, raceline = {raceline}")
        self._refresh_stanley_button()
        threading.Thread(target=self._read_stanley_output, daemon=True).start()

    def _stop_stanley(self):
        if not self.is_stanley_running():
            return
        try:
            os.killpg(os.getpgid(self.stanley_proc.pid), signal.SIGKILL)
        except ProcessLookupError:
            pass
        for pat in self.STANLEY_KILL_PATTERNS:
            subprocess.Popen(
                ["pkill", "-9", "-f", pat],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
        self.app.log_system(
            f"[Stanley] SIGKILL + pkill -9 sent "
            f"(patterns: {', '.join(self.STANLEY_KILL_PATTERNS)})")
        self._brake_motor(reason="Stanley stop")

    def _read_stanley_output(self):
        assert self.stanley_proc is not None and self.stanley_proc.stdout is not None
        for line in iter(self.stanley_proc.stdout.readline, ""):
            self.app.log_queue.put(("Stanley", line.rstrip()))
        self.stanley_proc.stdout.close()
        rc = self.stanley_proc.wait()
        self.app.log_queue.put(("__system__",
                                 f"[Stanley] exited with code {rc}"))
        self.app.root.after(50, self._on_stanley_exit)

    def _on_stanley_exit(self):
        self.stanley_proc = None
        self.stanley_paused = False
        self._refresh_stanley_button()
